...
export default {
  data() {
    return {
      userImg: {
        touxiang: require("../assets/images/5.png")
      },
      url: ""
    };
  },
  methods: {
    seen() {
      var userImg = JSON.parse(localStorage.userImg);
      console.log(userImg.avatar);
      // console.log(userImg.icon);
      this.url = userImg.avatar.replace(/public/, "http://47.102.135.95:1901");
    },
    updateimg() {
      console.log("上传图片");
    ...
    shangchuan(e) {
      ...
      //     this.userImg.touxiang = res.result
      // }
      ...
      //     localStorage.userImg = JSON.stringify({icon:res.data.imgUrl});
      // })
    }
  }
