...
<template>
  ...
  <van-dropdown-item v-model="value2" :options="option2" />
	...
	<router-link :to="{name:'detail',params:{id:m._id}}" v-for="(m,i) in shuig" style="width: 98%;display: block ;border: #0086B3 solid 1px; animation: bounceIn 1s;" v-lazy="m.pic" >
		<div @click="goods(m._id)">
		<img :src="m.pic" alt="" style="width: 100%;height: 3rem;">
		<p style="color:red">水果<span style="font-size: smaller;color: black;"> {{m.title}}</span></p>
		<p>价格:￥<span style="color: red;font-size: large;">{{m.price}}</span></p>
		</div>
	</router-link>
	</van-pull-refresh>
</div>
  ...
  methods: {
	...
	watch:{
		value1(v){
			...
			this.$axios.get("vue/fl",{params:{rex:this.option1[v].text}}).then(res=>{
